...
export async function ensureMarketOfferForFactory(
  ...
  const missingMarketSlots = Math.max(
    0,
    marketRules.stageRule.targetActiveOfferCount - activeOfferCount,
  );
  const dailySlots = Math.max(
    0,
    marketRules.stageRule.maxNewOffersPerDay - todayOfferCount,
  );
  const missingOfferCount = Math.min(missingMarketSlots, dailySlots);
...
async function fetchMarketRules(input: {
  ...
  return {
    ...
    stageRule:
      stageRule ?? getFallbackStageRule(input.currentStageSortOrder ?? 10),
  };
}
...
function buildPlannedOffer(input: {
  ...
  const targetProductionDays = seededInt({
    max: Math.max(
      input.customer.customerVolumeClass.targetProductionDayMin,
      input.customer.customerVolumeClass.targetProductionDayMax,
    ),
    min: input.customer.customerVolumeClass.targetProductionDayMin,
    seed: `${input.seed}:target-production-days`,
  });
...
function buildItemPlans(input: {
  ...
  const capacityBudgetPoints = Math.floor(
    (primaryProduct.bottleneckDailyQuantity *
      ...
      input.targetProductionDays *
      Math.max(1, input.customer.customerVolumeClass.maxOfferLoadBps) *
      Math.max(1, input.customer.customerVolumeClass.quantityMultiplierBps)) /
      100_000_000,
  );
  ...
  return shareWeights.flatMap(({ product, weight }, index) => {
    ...
    const maxLoadQuantity = Math.floor(
      (product.bottleneckDailyQuantity *
        input.targetProductionDays *
        Math.max(1, input.customer.customerVolumeClass.maxOfferLoadBps)) /
        10_000,
    );
...
function calculateTargetDeliveryDays(input: {
  ...
  const baseDeliveryDays = seededInt({
    max: Math.max(input.rule.minDeliveryDays, input.rule.maxDeliveryDays),
    min: input.rule.minDeliveryDays,
  ...
  return clamp(
    ...
    input.rule.minDeliveryDays,
    input.rule.maxDeliveryDays + collectionBufferDays,
  );
}
